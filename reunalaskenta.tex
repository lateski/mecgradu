...
Virtuaalikoneisiin pohjautuva cloudlet\cite{satya09} on yksi tunnetuimmista reunapalveluiden tuottamisen konsepteista.
Cloudletin ideana on hyödyntää tavanomaisia virtuaalikone-instansseja, joita suoritetaan reunasolmussa. 
Cloudlet on ajateltu käyttäjäkohtaiseksi ja yhdellä reunasolmulla voidaan saman aikaisesti suorittaa usean käyttäjän cloudletteja.

Virtuaalikoneiden käyttö cloudlettien perustana mahdollistaa helpon tavan jakaa reunasolmun resursseja ja pitää asikaskohtaiset instanssit toisistaan erillään. 
Suoritusaikana cloudletin käytössä alevat resurssit voidaan rajata samoin kuin perinteisenkin virtuaalikoneen.
Käyttäjäkohtaisien virtuaalikoneiden säilöminen reunasolmulla on kuitenkin mahdoton ajatus resurssien näkökulmasta.
Etenkin tilanteessa jossa käyttäjä haluaisi siirtää cloudlettinsä toiselle reunasolmulle jouduttaisiin koko virtuaalikoneen kuva siirtämään verkon yli reunasolmulta toiselle. 
Cloudlet ratkaisee ongelman siten että jokainen cloudlet pohjautuu cloudlet-pohjaan.
Pohja on tietynlainen lähtötila virtuaalikoneelle – kuin "puhdas-asennus".
Käyttäjän käyttäessä cloudlettiä, sen tilaan tulee muutoksia jotka "likaavat" puhtaan asennuksen.
Kun käyttäjä lopettaa cloudletin käyttämisen, halutaan cloudletin tila tallentaa käyttäjän paluuta varten. Cloudletin virtuaali-instanssia ei tallenneta kokonaisena vaan siitä tallennetaan ainoastaan käyttäjän siihen tekemät muutokset. Käyttäjän tekemät muutokset saadaan erotettua vertaamalla käyttäjän cloudlet-instanssia cloudlet-pohjaan. 
Muutokset tallennetaan tiedostoon joka voidaan pakata tiedostokoon minimoimiseksi.  
Käyttäjäkohtaisien muutoksien tallentaminen vie huomattavasti vähemmän tallennustilaa, kuin kokonaisen virtuaalikonekuvan tallentaminen.
Pienen tiedostokoon ansiosta käyttäjän cloudlet-kuvake on huomattavasti helpommin siirrettävissä.
Tiedostokoko on kertaluokkaa pienempi kuin kokonainen virtuaalikoneen kuva.
